...
class ThreeNaController extends Controller
{
    public function ThrNainput(){
        $school=schoolsession();
        $subject=Subject::where('class','Three')->where('babu','NA')->where('eiin',$school->eiin)->orderBy('subcode','asc')->get();
        return view('ThrNa.input',['school'=>$school,'subject'=>$subject]);
    }
 

  /// Subject  View Page
 public function ThrNasub($tecode){
  if(Session::has('school')){ 
       ...
       return view('ThrNa.'.$name->subcode,['school'=>$school,'subject'=>$subject,'name'=>$name ,'tecodesection'=>$tecodesection]);
         ...
         if(!empty(teacher_access($tecode,$subjectauth))){
              ...
              return view('ThrNa.'.$name->subcode,['name'=>$name,'tecodesection'=>$tecodesection]);
                }else{
   ...
   public function ThrNaSelect($tecodesection){


            if(Session::has('school')){ 
                $school=schoolsession();
                  $examinfo = Examinfo::where('babu','NA')->where('class','Three')->where('eiin',$school->eiin)->first();
              ...
              $student = DB::table('marks')
                 ...
                 ->where('marks.babu','NA')->where('marks.class','Three')->where('marks.section',substr($tecodesection,10,1))
...
public function ThrNasub_update(Request $request){
   ...
   $markinfo = Markinfo::where('babu','NA')->where('class','Three')->where('eiin',$school->eiin)->get();
   ...
   public function ThrNasub_update16(Request $request){
 ...
 public function ThrNaresult(Request $request){ 

    $school=schoolsession();
    $subject=Subject::where('class','Three')->where('babu','NA')->where('eiin',$school->eiin)->orderBy('subcode','asc')->get();
    $name=Subject::where('class','Three')->where('babu','NA')->where('eiin',$school->eiin)->first();
    $examinfo = Examinfo::where('babu','NA')->where('class','Three')->where('eiin',$school->eiin)->first();
    
    
     $mainsubject=Calculation::where('babu','NA')->where('class','Three')->where('eiin',$school->eiin)->first();
 
    $student = DB::table('marks')
      ->leftjoin('students', 'students.id', '=', 'marks.uid')
      ->where('marks.babu','NA')->where('marks.class','Three')->where('marks.section',Session::get('section'))
      ->where('marks.exam',$examinfo->exam)->where('marks.year',$examinfo->year)->where('marks.eiin',$school->eiin)
      ->select('students.name','students.stu_id','students.roll','students.moral','students.main','students.addi','marks.*')
    ...
    return view('ThrNa.result',['school'=>$school,'subject'=>$subject ,'name'=>$name,'student'=>$student,'tags'=>$tags,'student1'=>$student1]);
        }else{
         return redirect("/ThrNainput")->with('status','This section No Students OR No Subject Setup');
      }
   ...
   public function resulttype(Request $request){
        ...
        DB::update("update marks set rs ='$type'  where babu='NA' AND class='Three' AND section='$section'
        AND exam='$examinfo->exam' AND year='$examinfo->year' AND eiin='$school->eiin' ");
        return back()->with('success','Result Type Update ');
   }


 public function ThrNaresultupdate(Request $request){

   $school=School::where('eiin',Session::get('school')->eiin)->first();
   $examinfo=Examinfo::where('babu','NA')->where('class','Three')->where('eiin',$school->eiin)->first();
   $section=Session::get('section');

   $max=DB::table('marks')->where('babu','NA')->where('class','Three')->where('section',Session::get('section'))
   ...
   ,DB::raw("Max(sub23t) as sub23h"),DB::raw("Max(sub24t) as sub24h"))->first();
     ...
     $markinfo=Markinfo::where('babu','NA')->where('class','Three')->where('eiin',Session::get('school')->eiin)->get();
     $mainsubject=Calculation::where('babu','NA')->where('class','Three')->where('eiin',$school->eiin)->first();
     $subcode1=$mainsubject->subcode;  
     $tags=explode(',',$subcode1);
