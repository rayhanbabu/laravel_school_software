...
class EightNaController extends Controller
{
    public function EigNainput(){
        $school=schoolsession();
        $subject=Subject::where('class','Eight')->where('babu','NA')->where('eiin',$school->eiin)->orderBy('subcode','asc')->get();
        return view('EigNa.input',['school'=>$school,'subject'=>$subject]);
    }
 

  /// Subject  View Page
 public function EigNasub($tecode){
  if(Session::has('school')){ 
   ...
   public function EigNaSelect($tecodesection){
      if(Session::has('school')){ 
           $school=schoolsession();
           $examinfo = Examinfo::where('babu','NA')->where('class','Eight')->where('eiin',$school->eiin)->first();
       ...
       $student = DB::table('marks')
          ...
          ->where('marks.babu','NA')->where('marks.class','Eight')->where('marks.section',substr($tecodesection,10,1))
   ...
   public function EigNasub_update(Request $request){
   ...
   $markinfo = Markinfo::where('babu','NA')->where('class','Eight')->where('eiin',$school->eiin)->get();
   ...
   public function EigNasub_update16(Request $request){
        if(Session::has('school')){ 
                 $school=schoolsession();
        }else if(Session::has('teacher')){
                 $school=teachersession();
 ...
 public function EigNaresult(Request $request){
         
    $school=schoolsession();
    $subject=Subject::where('class','Eight')->where('babu','NA')->where('eiin',$school->eiin)->orderBy('subcode','asc')->get();
    $name=Subject::where('class','Eight')->where('babu','NA')->where('eiin',$school->eiin)->first();
    $examinfo = Examinfo::where('babu','NA')->where('class','Eight')->where('eiin',$school->eiin)->first();
    
    
     $mainsubject=Calculation::where('babu','NA')->where('class','Eight')->where('eiin',$school->eiin)->first();
     $subcode1=$mainsubject->subcode;  
     $tags=explode(',',$subcode1);

     $student = DB::table('marks')
      ->leftjoin('students', 'students.id', '=', 'marks.uid')
      ->where('marks.babu','NA')->where('marks.class','Eight')->where('marks.section',Session::get('section'))
      ->where('marks.exam',$examinfo->exam)->where('marks.year',$examinfo->year)->where('marks.eiin',$school->eiin)
      ->select('students.name','students.stu_id','students.roll','students.moral','students.main','students.addi','marks.*')
      ...
      if($student1){    
     return view('EigNa.result',['school'=>$school,'subject'=>$subject ,'name'=>$name,'student'=>$student,'tags'=>$tags,'student1'=>$student1]);
    
      }else{
           return redirect("/EigNainput")->with('status','This section No Students');
       }
    }
  ...
  public function resulttype(Request $request){
    ...
    DB::update("update marks set rs ='$type'  where babu='NA' AND class='Eight' AND section='$section'
 ...
 public function EigNaresultupdate(Request $request){

     $school=School::where('eiin',Session::get('school')->eiin)->first();
   $examinfo=Examinfo::where('babu','NA')->where('class','Eight')->where('eiin',$school->eiin)->first();
   $section=Session::get('section');

   $max=DB::table('marks')->where('babu','NA')->where('class','Eight')->where('section',Session::get('section'))
   ...
   ,DB::raw("Max(sub23t) as sub23h"),DB::raw("Max(sub24t) as sub24h"))->first();
     ...
     $markinfo=Markinfo::where('babu','NA')->where('class','Eight')->where('eiin',Session::get('school')->eiin)->get();
