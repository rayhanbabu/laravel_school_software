...
class NineScController extends Controller
{
     public function NinScinput(){
          $school=schoolsession();
          $subject=Subject::where('class','Nine')->where('babu','Science')->where('eiin',$school->eiin)->orderBy('subcode','asc')->get();
          return view('NinSc.input',['school'=>$school,'subject'=>$subject]);
      }
 

  /// Subject  View Page
 public function NinScsub($tecode){
  if(Session::has('school')){ 
       ...
       return view('NinSc.'.$name->subcode,['school'=>$school,'subject'=>$subject,'name'=>$name ,'tecodesection'=>$tecodesection]);
  ...
  public function NinScSelect($tecodesection){
   if(Session::has('school')){ 
        $school=schoolsession();
        $examinfo = Examinfo::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->first();
    ...
    $student = DB::table('marks')
       ...
       ->where('marks.babu','Science')->where('marks.class','Nine')->where('marks.section',substr($tecodesection,10,1))
...
public function NinScsub_update(Request $request){
     ...
     $markinfo =Markinfo::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->get();
 ...
 public function NinScsub_update12(Request $request){
...
public function NinScsub_update14(Request $request){
  ...
  if(Session::has('school')){ 
   ...
   public function NinScsub_update16(Request $request){
      ...
      $markinfo = Markinfo::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->get();
          ...
          $tsubcode=$subcode.'code'; 
           ...
           $grade= grade($subc,$cfail,$subm,$mfail,$subp,$pfail,$total,$markinfo,$tmark);
                ...
                DB::update(
                    "update marks set $tsubc ='$subc', $tsubm ='$subm', $tsubp ='$subp' , $tsubt ='$total',
                    $tsubgp='$gpa', $tsubg='$grade', $tsubg='$grade' , $tsubcode='$subid' 
    ...
    public function NinScsub_update24(Request $request){
 ...
 public function NinScresult(Request $request){
         
    $school=schoolsession();
    $subject=Subject::where('class','Nine')->where('babu','Science')->where('eiin',$school->eiin)->orderBy('subcode','asc')->get();
    $name=Subject::where('class','Nine')->where('babu','Science')->where('eiin',$school->eiin)->first();
    $examinfo = Examinfo::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->first();
    
    
     $mainsubject=Calculation::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->first();
     $subcode1=$mainsubject->subcode;  
     $tags=explode(',',$subcode1);

    $student = DB::table('marks')
      ->leftjoin('students', 'students.id', '=', 'marks.uid')
      ->where('marks.babu','Science')->where('marks.class','Nine')->where('marks.section',Session::get('section'))
      ->where('marks.exam',$examinfo->exam)->where('marks.year',$examinfo->year)->where('marks.eiin',$school->eiin)
      ->select('students.name','students.stu_id','students.roll','students.moral','students.main','students.addi','marks.*')
      ...
      $student1=$student->first();
          ...
          if($student1){  
    return view('NinSc.result',['school'=>$school,'subject'=>$subject ,'name'=>$name,'student'=>$student,'tags'=>$tags,'student1'=>$student1]);
          }else{
         return redirect("/NinScinput")->with('status','This section No Students');
      }
 ...
 public function resulttype(Request $request){
     ...
     DB::update("update marks set rs ='$type'  where babu='Science' AND class='Nine' AND section='$section'
     AND exam='$examinfo->exam' AND year='$examinfo->year' AND eiin='$school->eiin' ");
     return back()->with('success','Result Type Update ');
 }


  public function NinScresultupdate(Request $request){

    $school=School::where('eiin',Session::get('school')->eiin)->first();
    $examinfo=Examinfo::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->first();
    $section=Session::get('section');

    $max=DB::table('marks')->where('babu','Science')->where('class','Nine')->where('section',Session::get('section'))
    ...
    ,DB::raw("Max(sub23t) as sub23h"),DB::raw("Max(sub24t) as sub24h"))->first();
     ...
     $markinfo=Markinfo::where('babu','Science')->where('class','Nine')->where('eiin',Session::get('school')->eiin)->get();
     $mainsubject=Calculation::where('babu','Science')->where('class','Nine')->where('eiin',$school->eiin)->first();
     $subcode1=$mainsubject->subcode;  
     $tags=explode(',',$subcode1);
