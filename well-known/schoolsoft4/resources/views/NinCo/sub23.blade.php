  ...
  <form  method="POST" id="update_form" enctype="multipart/form-data">
   <table class="table table-bordered" style="font-size:15px;" >
     ...
     <tr style="background: whitesmoke;">
               ...
               <input type="hidden" name="subject" value="main"  class="form-control">
...
$(document).ready(function(){  
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            var html = '';
                ...
                {
                    if(response.data[count].sub23c==0){ var subc=''; }else{
              var subc=response.data[count].sub23c; }

                    if(response.data[count].sub23m==0){ var subm=''; }else{
             var subm=response.data[count].sub23m; } 
                    
                    if(response.data[count].sub23p==0){ var subp=''; }else{
              var subp=response.data[count].sub23p; }
   ...
   html += '<td>'+response.data[count].sub23n+'</td>';
   html += '<input type="hidden" name="shortsub[]" class="form-control" value="'+response.data[count].main+'"/>';
   ...
   html += '<td>'+response.data[count].sub23t+'</td>';
   html += '<td>'+response.data[count].sub23gp+'</td>';
   html += '<td>'+response.data[count].sub23g+'</td>';
   html += '</tr>';
